...
class Encoder(nn.Module):
    def __init__(self,in_feats, n_hidden, nclass, activation, dropout):
        ...
        # )
        self.fc1 = GraphConvolution(in_feats, nclass)
...
class DGI(nn.Module):
    ...
    def forward(self, features,adj,domain='t',mask=None):
        ...
        if mask!=None:
            summary = torch.sigmoid(positive_ori[mask].mean(dim=0))
            positive = self.discriminator(positive_ori[mask], summary)
            negative = self.discriminator(negative_ori[mask], summary)
        else:
